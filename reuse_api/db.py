...
def is_lockable(repo: str, timeout: int = UPDATE_TIMEOUT) -> bool:
    ...
    if not exists(lockfile):
        return True
    # If lock timed out
    age: int = time() - getmtime(lockfile)
    return age >= timeout


def __lock(repo) -> bool:
...
def _not_updated() -> list[str]:
    ...
    return _get_registered_repos(
        lambda dirpath, dirnames, filenames: not (filenames or dirnames)
    )
...
def _outdated(age_in_seconds: int = 24 * 60 * 60) -> list[str]:
    ...
    return _get_registered_repos(
        lambda dirpath, dirnames, filenames: isfile(f"{dirpath}/{__HEAD_FILE}")
